...
<%
	...
	String foundEmail = request.getParameter("email");
	...
	if (foundEmail != null) {
		...
		foundEmail = sb.toString();
	}
	
	String foundTel = request.getParameter("tel");
			...
			<form method="post" class="border py-5 px-5 mt-4" id="find-password-form" action="findPassword.jsp" >
				...
				%>		
						<div class="form-check">
							...
							<label class="form-check-label" for="found-email">이메일 인증 ( <%=foundEmail%> )</label>
						</div>
						<div class="form-check">
							<input class="form-check-input" type="radio" name="findWay" id="found-tel" value="tel" />
							<label class="form-check-label" for="found-tel">휴대폰 인증 ( <%=foundTel %> )</label>
						</div>
				<%
					} else {
				%>
		...
		let xhr = new XMLHttpRequest();
			xhr.onreadystatechange = function() {
				...
				if(xhr.readyState === 4 && xhr.status === 200) {
					...
					if(result.exist) {
						formField.action = "findPassword.jsp?id=" + result.id + "&email=" + result.email + "&tel=" + result.tel;
						formField.submit();
						return;
